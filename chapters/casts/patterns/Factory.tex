...
(line 10).
The variable \code{keyPair} is assigned the result of \code{pairGen.generateKey\-Pair} (line 3).
...
the next example shows an instance of the \thisp{} pattern where a cast is performed on the result of the \code{openConnection} method%
...
since the the statement is a \code{SELECT} statement,
...
the authors had to look-up the method implementation in external source code repositories.
