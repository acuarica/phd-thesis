...
\detection{}
\done{Matthias: If you do it manually, I wouldn't talk of a detection *query*.}
We look for a cast expression inside an \code{equals} method implementation.
Moreover, the cast needs to be guarded by either an \code{instanceof} test or a \code{getClass} comparison.
And the type being cast needs to be either the same as the enclosing class or a superclass of it.
...
\related{}
This pattern can be seen as a special instance of the \nameref{pat:PatternMatching} pattern when the guard is an \code{instanceof} test.
