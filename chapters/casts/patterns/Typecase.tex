...
It was taken from a \lgtm{} rule.%

\footnote{\url{https://lgtm.com/rules/910065/}}

\begin{listing}
\begin{minted}{java}
class IofControlsVarCast extends VarCast { #\qlbox#
    InstanceOfExpr iof;
...
class IofGuardsVarCast extends IofControlsVarCast {
    IofGuardsVarCast() {
...
final class GuardByInstanceOfCastPattern extends CastPattern {
	GuardByInstanceOfCastPattern() {
		this instanceof IofGuardsVarCast
	}
	override string getPattern() { result = "GuardByInstanceOf" }
}
\end{minted}
...
\begin{listing}
\begin{minted}{java}
final class GuardByTypeTagCastPattern extends CastPattern { #\qlbox#
	private FieldAccess tagAccess;
	private FieldAccess castAccess;
	private Variable v;
	GuardByTypeTagCastPattern() {
		tagAccess = this.(SwitchedExpr).getSwitchStmt().getExpr() and
		castAccess = getExpr() and
		v.getAnAccess() = tagAccess.getQualifier() and
		v.getAnAccess() = castAccess.getQualifier()
	}
	override string getPattern() { result = "GuardByTypeTag" }
	FieldAccess getTagAccess() { result = tagAccess }
	FieldAccess getCastAccess() { result = castAccess }
	Variable getVariable() { result = v }
}
\end{minted}
\caption{Query for the \variant{GuardByTypeTag} variant.}
\end{listing}
