...
That is, the result of a \ql{} query%
---cast expressions in the AST in our case---%
for a pattern, most often belong to that pattern.
Moreover, there could be others cast expressions not returned by the query,
which are instances of the pattern.
This is a \emph{best-effort} approach.
We believe that is better to have at least this kind of automatic approximation than nothing at all.

Unfortunately,
we do not possess the \lgtm{} database.
It is not feasible for us to run our queries on their database.
Therefore, it is impossible to gather partial statistics using our queries.
