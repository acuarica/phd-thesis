...
For instance, \java{} provides the cast operator, \eg{}, \code{(T) t},
while \scala{} provides the \code{asInstanceOf} method,
\eg{}, \code{t.asInstanceOf[T]}.
Although \cpp{} is an unsafe language,
when used with run-time type information (RTTI),
it provides a safe cast operation%
---the \code{dynamic\_cast<>} operator---%
\eg{}, \code{dynamic\_cast<T>(t)}.
However, being unsafe,
\cpp{} provides more ways to perform a cast operation with different semantics, \ie{},
\code{static\_cast<>},
...
or when casting to a class that is \emph{not} a supertype of the object's \emph{run-time} class.
