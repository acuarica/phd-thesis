...
% DONE: made a study -> conducted a study
% DONE: Avoid "note that", "notice that"

% DONE: Static vs. Dynamic analysis in the literature review
% DONE: Why static, why not dynamic? where static is OK, where is reasonable, feasible?

% DONE: Refactor this chapter into four subsections:
...
The first one
% when conducting empirical studies of programs
is 
...
This is presented in \S\ref{sec:lr:benchmarks}.
...
In \S\ref{sec:lr:largescale}, we give an overview of large-scale
empirical studies
% either in \java{} or other languages
that show what
...
\S\ref{sec:lr:unsafe} and \S\ref{sec:lr:casting}
respectively.
...
% DONE: OOPSLA'17 code duplicates in GitHub
% DONE: SPEC, etc
...
% DONE: Some discussion of the representativeness of the corpora
...
% DONE: Acknowledge here that open source repositories, while not curated corpora, are very commonly used.
...
% DONE: How do clones fit in this section?
% to code duplication detection is code 
...
% DONE: Instead of this, add an intro sentence motivating this section by connecting it to your focus on type safety (if possible).
% DONE: Generics is not just a functional language feature, no?
...
% DONE: Moved above to show only that there is work using Unsafe.
...
throwing \code{ClassCastException}.
